...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'Orchestify', separator: { label: ' - ' } }), 'Orchestify');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'Orchestify', separator: { label: ' - ' } }), 'Untitled-1 - Orchestify');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'Orchestify', separator: { label: ' - ' } }), 'monaco - Orchestify');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Orchestify', separator: { label: ' - ' } }), 'somefile.txt - monaco - Orchestify');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Orchestify', separator: { label: ' - ' } }), '* somefile.txt - monaco - Orchestify');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'Orchestify', separator: { label: ' - ' } }), 'monaco - Orchestify');
	});
