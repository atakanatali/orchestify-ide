...
export class PromptExtensionInstallFailureAction extends Action {
	...
	override async run(): Promise<void> {
		...
		if (this.error.name === ExtensionManagementErrorCode.Unsupported) {
			const productName = isWeb ? localize('Orchestify for Web', "{0} for the Web", this.productService.nameLong) : this.productService.nameLong;
		...
		if (ExtensionManagementErrorCode.SignatureVerificationFailed === (<ExtensionManagementErrorCode>this.error.name)) {
			await this.dialogService.prompt({
				...
				buttons: [{
					...
					run: () => this.openerService.open('https://code.visualstudio.com/docs/editor/extension-marketplace#_the-extension-signature-cannot-be-verified-by-vscode')
				}, {
	...
	override async run(): Promise<any> {
		...
		if (this.extension.deprecationInfo) {
			...
			if (this.extension.deprecationInfo.extension) {
				...
				detail = localize('deprecated with alternate settings message', "This extension is deprecated as this functionality is now built-in to Orchestify.");
...
export class UninstallAction extends ExtensionAction {
	...
	override async run(): Promise<any> {
		...
		try {
			...
			alert(localize('uninstallExtensionComplete', "Please reload Orchestify to complete the uninstallation of the extension {0}.", this.extension.displayName));
		} catch (error) {
	...
	private async computeAndUpdateStatus(): Promise<void> {
		...
		if (this.extension.deprecationInfo) {
			if (this.extension.deprecationInfo.extension) {
				...
				this.updateStatus({ icon: warningIcon, message: new MarkdownString(localize('deprecated with alternate settings tooltip', "This extension is deprecated as this functionality is now built-in to Orchestify. Configure these {0} to use this functionality.", link)) }, true);
			} else {
